...
def black_formula(
        ...
        std_dev: npt.ArrayLike,
        discount: npt.ArrayLike
):
    d1 = np.log(forward/strike)/std_dev + 0.5*std_dev
    d2 = d1 - std_dev
    nd1 = norm.cdf(int(option_type) * d1)
    nd2 = norm.cdf(int(option_type) * d2)
    return  discount * option_type * (forward*nd1 - strike*nd2)



#def delta(spot):
#    assert spot > 0.0, "positive spot value required: " << spot << " not allowed"
#    DforwardDs = forward_ / spot
#    temp = stdDev_* spot
#    DalphaDs = DalphaDd1/temp 
##    DbetaDs  = DbetaDd2/ temp
#    temp2 = DalphaDs * forward_ + alpha_ * DforwardDs +DbetaDs  * x_ + beta_ * DxDs_
